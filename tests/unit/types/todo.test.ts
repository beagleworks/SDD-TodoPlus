...
import type { TodoAction } from '../../../src/types/todo'
import {
  validateTodo,
  validateTodoState,
...
describe('Todo Type Validation', () => {
  ...
  describe('TodoAction type safety', () => {
    it('should fail type checking for invalid TodoAction', () => {
      ...
      expect(
        processAction({ type: 'ADD_TODO', payload: { title: 'Test' } })
      ).toBe('Adding todo: Test')
      expect(
        processAction({ type: 'DELETE_TODO', payload: { id: 'test-id' } })
      ).toBe('Deleting todo: test-id')
      expect(
        processAction({ type: 'SET_FILTER', payload: { filter: 'completed' } })
      ).toBe('Setting filter: completed')
    })
    ...
    it('should ensure TodoAction payload type safety', () => {
        ...
        switch (action.type) {
          ...
          case 'UPDATE_TODO':
            return (
              ...
              typeof action.payload.updates === 'object'
            )
          case 'DELETE_TODO':
          ...
          case 'SET_FILTER':
            return ['all', 'not_started', 'in_progress', 'completed'].includes(
      ...
      expect(
        validateActionPayload({
          type: 'UPDATE_TODO',
          payload: { id: 'test-id', updates: { title: 'Updated' } },
        })
      ).toBe(true)
      expect(
        validateActionPayload({
      ...
      expect(
        validateActionPayload({ type: 'REORDER_TODOS', payload: { todos: [] } })
      ).toBe(true)
      expect(
        validateActionPayload({
          type: 'SET_FILTER',
      ...
      expect(
        validateActionPayload({ type: 'LOAD_TODOS', payload: { todos: [] } })
      ).toBe(true)
    })
  })
