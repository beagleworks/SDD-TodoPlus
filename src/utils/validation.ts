...
export const TodoSchema = z.object({
  ...
  completionComment: z
    .string()
    .max(500, 'Completion comment must be 500 characters or less')
    .optional(),
  createdAt: z.date(),
  updatedAt: z.date(),
...
export function validateLocalStorageData(data: unknown): {
  todos: Todo[]
  version: string
} {
  try {
...
export function isValidFilterType(
  filter: string
): filter is TodoState['filter'] {
